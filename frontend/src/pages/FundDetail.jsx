...
import { FUNDS, formatTRY, formatPct, formatNum } from '../data/mock';
...
const FundDetail = () => {
  ...
  const { holdings } = useAuth();
  const [range, setRange] = useState('1A');

  const fund = useMemo(() => FUNDS.find((f) => f.code === code), [code]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="text-xs text-slate-500">{fund.categoryLabel}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 fa-card fa-glow p-6">
          <div className="flex items-end justify-between mb-4">
            <div>
              ...
              <div className="flex items-baseline gap-3">
                ...
                <div className={`text-sm font-semibold ${positive ? 'fa-positive' : 'fa-negative'}`}>{formatPct(fund.change24h)}</div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 mt-6 pt-6 border-t border-slate-100">
            <Stat label="YBB Getiri" value={<span className={fund.changeYtd >= 0 ? 'fa-positive' : 'fa-negative'}>{formatPct(fund.changeYtd)}</span>} />
          ...
          {myHolding && (
            <div className="fa-card fa-glow p-5 border-l-4 border-l-emerald-500">
              ...
              <div className="grid grid-cols-2 gap-3 mt-4 text-xs">
                ...
                <div><div className="text-slate-500">Ort. Maliyet</div><div className="font-semibold text-[#0B2447]">{myHolding.avgCost.toFixed(4)}</div></div>
                <div><div className="text-slate-500">K/Z</div><div className={`font-semibold ${fund.price >= myHolding.avgCost ? 'fa-positive' : 'fa-negative'}`}>{formatTRY((fund.price - myHolding.avgCost) * myHolding.units)}</div></div>
                <div><div className="text-slate-500">K/Z %</div><div className={`font-semibold ${fund.price >= myHolding.avgCost ? 'fa-positive' : 'fa-negative'}`}>{formatPct(((fund.price - myHolding.avgCost) / myHolding.avgCost) * 100)}</div></div>
              </div>
            </div>
          )}
